...
void zclSampleSwOta_Init( byte task_id )
{
  ...
  queryImgJitter = ( (uint32)osal_rand() % 299000 ) + (uint32)300000; 
  osal_start_reload_timer( zclSampleSwOta_TaskID, SAMPLESWOTA_QUERY_SERVER_EVT, queryImgJitter );
#endif // OTA_CLIENT
...
uint16 zclSampleSwOta_event_loop( uint8 task_id, uint16 events )
{
  ...
  if ( events & SAMPLESWOTA_QUERY_SERVER_EVT )
  {
    // if an OTA update is already in progress, we can ignore this event.
    if (zclOTA_getStatus() != OTA_STATUS_IN_PROGRESS)
    {
      zclOTA_RequestNextUpdate( zclSampleSwOta_DstAddr.addr.shortAddr, zclSampleSwOta_DstAddr.endPoint );
    }
    return ( events ^ SAMPLESWOTA_QUERY_SERVER_EVT );
  }
...
static void zclSampleSwOta_HandleKeys( byte shift, byte keys )
{
  ...
  {
#if defined HAL_IMG_AREA && defined HAL_MCU_CC2538
    // This is to demonstrate the easy ping-pong between two run-in-place images.
    preamble_t currImg, nextImg;
    HalOTARead(PREAMBLE_OFFSET, (uint8 *)&currImg, sizeof(preamble_t), HAL_OTA_RC);
    HalOTARead(PREAMBLE_OFFSET, (uint8 *)&nextImg, sizeof(preamble_t), HAL_OTA_DL);

    HalLcdWriteString("Images Ping-Ponging:",                    HAL_LCD_LINE_1);
    HalLcdWriteStringValue("Retiring", currImg.imageVersion, 10, HAL_LCD_LINE_2);
    HalLcdWriteStringValue("Aspiring", nextImg.imageVersion, 10, HAL_LCD_LINE_3);

    HalOTAInvRC();

    for (uint32 dly = 0; dly < 0x00C00000; dly++)
    {
      asm("NOP");
    }
    SystemReset();
